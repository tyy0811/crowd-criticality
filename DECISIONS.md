...
**Why (measured, 8 seeds).** Resolution `Δ_min = 2√2·SD_seed(Δ)`: **0.49 (20k) → 0.24 (100k) → 0.12 (400k)**. Two facts: (1) `Δ_crit → 0` with N (+0.17 → +0.01) — the bias floor is dead, reconfirmed. (2) `Δ_min` is **coarse and intrinsic**: it's set by the **CSN marginal-exponent (`τ, α`) seed-variance**, which `(α−1)/(τ−1)` amplifies — *not* the `1/σνz` estimator. So no better `1/σνz` estimator fixes it. A.2 catches gross violations (5–10% shuffle, `V~0.2–0.5`) but cannot resolve fine near-critical distinctions (`Δ < ~0.15`) at the counts most prediction markets have (~1e3–1e5; only the largest reach 1e6).
