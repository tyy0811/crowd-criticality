...
4. **Omitted eligibility finding (important):** complete same-action opportunity is now MEASURED as recipient-signed-up AND parent-is-root-post (the installed platform redirects quote/repost of derived posts to their root, so only a root parent can natively receive all three registered same-actions); a derived parent fails provenance.
5. **Plan amendment (important):** the plan's registered Task-8 command now carries `--manifest <banked dry-run manifest>` as a dated prospective amendment, written before any authorization or execution.
...
- The frame and evidence hashes reproduced **byte-identically across three independent full-scale dry-runs** (2026-07-22 pre-repair; 2026-07-23 after the first repair round; 2026-07-23 after the second repair round, including the opportunity-semantics change — reply-layout parents are all roots, so no evidence boolean moved, measured not assumed). The manifest hash `3553d946…` is stable across both post-re-bank runs; durable copies of both banked manifests sit in `~/crowd-crit-runs/s4_causal_probe/`.
...
- Whole-repo non-slow suite at the round-2-repaired tree: **530 passed, 2 registered skips, 44 deselected slow** (`pytest -q -m "not slow"`), `git diff --check` clean. Focused 12-file causal battery: **335 passed**.
