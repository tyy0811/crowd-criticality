...
4. **Omitted eligibility finding (important):** complete same-action opportunity is now MEASURED conservatively as recipient-signed-up AND parent-is-root-post — the case for which all three registered same-actions (comment on the post; `quote_post`/`repost` of a `common`/root post, which record `original_post_id = parent`) land natively on the parent, whereas the platform's quote/repost of a derived source resolves to its `root_post_id`. A derived parent fails provenance. (Stated as the conservative sufficient condition, not a blanket claim about every post type's redirect behavior.)
