...
> **[SUPERSEDED 2026-07-29 — see §9.]** The "~1–1.5 hours" and "lever 2 saves only ~20 min" claims in this section are measured-false. The authorized run measured each reply cell rebuilding the full 24,578-agent platform in ~14.25 min → ~17–20 h for the grid. §9 records the run, the abort, and the corrected accounting.

## 9. Task-8 authorized run (2026-07-29) — measured ~17–20 h blowout, clean abort; §8 feasibility superseded

The owner authorized the recoverability grid at `cfff26b` under a bounded five-step protocol (fresh dry-run → manifest-SHA hard gate → single registered `--execute` → exit-0 + frozen structural/hash gates → stop on any failure), with the re-confirmed `--manifest` amendment, on the stated ~1–1.5 h / $0 budget. No LLM pilot, merge, or push was authorized. This section records the run and corrects §8; it authorizes nothing further.

- **Steps 1–2 PASSED.** A fresh full-scale dry-run reproduced the banked manifest `3553d946…` **byte-exact** (hard gate), with power `b4091ab1…`, frame `045bd40c…`, evidence `9980fbc1…`, support 8192×4, `provider_inference_fail_closed:true`/`invoked:false` all matched in-manifest. Dry-run wall ≈ 22 min (one full platform build + ~54 s sim; consistent with lever-1 roughly halving the 2026-07-23 ≈44-min dry-run — the build term, not the poll, now dominates it).
- **Step 3 measured a per-cell full rebuild.** `--execute` built one full 24,578-agent platform in **14 m 15 s** (15:24:08→15:38:23, 13.45 MB DB), then reply cell 1 (`reply_0.6_2026072201`) began a *fresh* full rebuild — 94 KB pre-commit, silent ~9.5 min, **not** reusing the predraw platform — when I aborted at ≈ 24 min in, cell 1 of 84. **No result artifact was written** (writer reached only at full completion; verified absent). Worktree clean at `cfff26b`.
- **Diagnosis (read-only, owner-confirmed).** `run_execute` invokes `run_scripted_oasis_control` **once per reply seed**; each invocation constructs a fresh `Platform`/`AgentGraph`, runs `env.reset()` over all 24,578 agents, and recreates all 16,386 posts. **72 reply runs × ~14.25 min ≈ 17.1 h of rebuilds alone**; the 12 marker runs use a small five-agent platform (not full rebuilds); with predraw + reply execution + markers, **~17–20 h is credible**.
- **Why §8 was wrong.** §8's "72 reply runs → ~44 min" and lever-2 "~20 min" figures were an **action-cost extrapolation, not an integrated 72-run measurement** — they omitted the dominant full-scale `env.reset()`/build term. The 303-agent transport A/B cannot expose per-agent build scaling, and the ~22-min full-scale dry-run (itself dominated by one ~14-min build) should have invalidated the forecast. **The integrated-measurement acceptance gate was not actually satisfied.** The "~20 min" also contradicted the lever-2 spec's own "≈45 min × 72 ≈ 2.25 days" rebuild term. Same class of error as the original 2026-07-23 mis-forecast.
- **Decision (owner).** Do not rerun; the one-time authorization is **consumed** and the Task-8 owner stop is binding again. **Lever 2 → reopened design candidate, not approved for implementation** (post-lever-3 the rebuild is the dominant remaining cost — precisely what cold-attach targets — but its spec returns through review before any code). Nothing in the repo or instrument changed.
- **Durable evidence** (scratch recorded here so it may be removed): execute log `~/crowd-crit-runs/s4_causal_probe/2026-07-29_run/execute_2026-07-29.log` SHA `991597952dc91958eaec86de9ab419d71a262fa8792f2a0848616e8357806ac9`; dry-run log `dryrun_2026-07-29.log` SHA `7b1614ebc079fdc9884c5ece6e68f8e7cb1af50ada2c2586fbcc4e4d302c3b47`; banked manifest `dryrun_manifest.json` SHA `3553d946…`; scratch `…/T/causal_probe_recoverability_kmofgpee/predraw_verification.db` 13,455,360 B SHA `75fb51d22f7db59227479587abc54383eea55dd20d47d04646a172f8fe11dc5e`, `reply_0.6_2026072201.db` (incomplete) 94,208 B SHA `bbeec502fa30ffbf7cdea5164c23be4f5632d5d7485998445e76ab8e1dc5f437`.
