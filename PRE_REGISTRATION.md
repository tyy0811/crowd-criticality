...
## 11. Local prospective registration — OASIS parrot null v2: paired structural invariance of the exposure axis (Branch B; registered 2026-08-17)

**Scope and governance.** This is a **local, prospective** registration of one bounded, $0 null-confirmation measurement under the activated Branch B ("observable knob-space and null confirmation without `n` placement or H1b" — `causal_probe_spec.BRANCH_B_TEXT`; activation: DECISIONS.md 2026-07-30). Per the governance notice above, it carries **local scope only**: it does not repair, freeze, or alter the global DRAFT status of this document, fills no global placeholder, and changes nothing in §§0–10 — in particular §4f's historical parrot-null `[fill]` remains historical and is NOT retro-filled. It is registered **before** the measurement it governs has been authorized or run. Registered sources, transcribed here without modification: the signed design (rev 5, 2026-07-31; git-ignored living doc `docs/superpowers/specs/2026-07-30-branchb-parrot-null-design.md`, SHA-256 `1b8fe8cfa51f935dd8ff6e7fe1710616bee74685301e418f3132bc650c670ac6`, pinned in `parrot_v2_spec.DESIGN_DOC_SHA256`), the signed implementation plan (rev 3, 2026-07-31), and the owner-accepted implementation at commit `b7d5eb5` (Tasks 0–8; frozen surface `src/critaudit/sim/controls/parrot_v2_spec.py`).

**Question (frozen).** Under knob-independent, content-blind serial replay of identical pre-generated action schedules, is the OASIS harness's emission-side output — the full exported `EventRecord`s (including `seq`) and the derived `n_events`, `frac_size1`, `giant_frac`, `length_spread` — exactly invariant to `refresh_rec_post_count ∈ {1, 3, 5}`, given direct trace-level evidence that the exposure manipulation operated? Expected reading: invariance HOLDS. A violation is a measured exposure→emission coupling in the platform or export pipeline. `read_emit_ratio` is an exposure/manipulation **diagnostic, expected to vary** with width (its success condition requires the parent in the served set), and is never a flatness outcome.

**Substrate and grid (frozen constants, `parrot_v2_spec.py`).** Platform surface = the registered cohort's, verbatim (`OPERATING_POINT`: 50 agents, 20 rounds, density 0.10 → M=245 exact directed edges, news user id 50; `RECSYS_TYPE="random"`; `FOLLOWING_POST_COUNT=3`; driver `MAX_REC_POST_LEN=5`; stock channel; fail-closed sentinel — any LLM invocation raises). Only the serving width varies: {1, 3, 5} = shipped endpoints + ratified midpoint. Schedule seeds `818201–818212` (12, shared across all widths — paired design); violator schedules `818201–818203` (one per cohort window under round-robin assignment over `COHORT_SEEDS`); **45 runs = 36 invariance (12×3) + 9 violator (3×3)**. v2 RNG namespace: SeedSequence spawn key `(818, stream)`, streams content/type/agent/target/refresh/platform = 0–5, disjoint from every registered stream; one platform seed per schedule, derived from stream 5, **reused across the width triple**, applied by seeding global `random` in a fresh subprocess per replay.

**Schedules (design §3, as implemented).** Knob-independent hashed `ScheduleEnvelope` (schema, schedule seed, assigned window, profile SHA-256, platform seed, actions, schedule SHA-256 — non-circular; validated and echoed by every replay). Per-window profiles via the whitelisted, hash-pinned extraction (per-round type counts news-adjusted — each news slot subtracts from the round total AND its round's `create_post` count; traced refresh counts; news events; non-news non-repost authored corpus). Content rule, three-way: news slots verbatim; repost slots `content=""` (reposts are pure structural re-emissions; the strict exporter rejects authored text on them); all other crowd-authored slots bootstrap uniformly from the corpus. Within-round order frozen: refreshes first (generated order), `create_post` slots first within the crowd block, remaining crowd slots in seeded order, news last. Targets: prior post-type emissions only (comments never targets); repost exclusion is the literal two-clause predicate mirroring the platform's duplicate checks including repost-to-root resolution; an empty action-valid target set fails at schedule generation, before replay. The v1 matched-null do-not-match list binds unchanged (branching ratio, avalanche-size distribution, per-agent emission rate never matched).

**Replay protocol (design §3b, as implemented).** Serial equivalent of the registered driver, per round r: assert `sandbox_clock.time_step == r` → one `update_rec_table()` → the round's actions strictly sequentially in schedule order via the agent action path → increment the clock exactly once → assert `r+1`. `env.step()` is never called in the replay loop. Refresh outcomes are recorded in a ledger keyed by immutable schedule-action index (served and empty alike; the empty return `"No posts found."` writes no trace row and is recorded-not-fatal); served entries are cross-checked 1:1 against the strict loader.

**Pass criterion and readings (frozen; no statistics).** Per schedule: canonical emission-record bytes (all fields incl. `seq`) AND the four gated observables exactly equal across the three widths. Guards: the manipulation guard (frozen width-triple pin; ledger skeleton alignment; zero paired served-set differences = "no realized exposure contrast; invalid for this schedule"; missing/misaligned/outcome-flip = invalid, never contrast) and the deterministic violator (one additional post after EVERY scheduled refresh whose content is the SHA-256 of the canonical served-set encoding, empty set included; pairwise: every width pair with served contrast must differ in emission bytes). Readings: **R1** null CONFIRMED (all 12 schedules invariant, guards hold, violator behaves); **R2** measured exposure→emission coupling (reported verbatim; nothing repaired, re-run, or reinterpreted); **R3** auditable/reportable rig finding (any run/schema/digest/identity/alignment/guard failure). **Precedence frozen: R3 conditions dominate R2 even when an emission mismatch coexists.**

**Failure/stop rules and claim boundaries (design §6).** Fail-stop on any exception; no tuning, no adaptive reruns, no cell added/dropped/reordered; partials labelled non-registered. Results MAY support: the R1/R2/R3 verdict for this pipeline on this substrate at these widths under this replay class; recorded exposure diagnostics; rig findings. MAY NOT support: criticality or near-criticality claims; Hawkes-`n` placement or equivalence; H1b; `chi_resp`-on-OASIS transfer; mechanism claims about "coupling strength" (the exposure-volume caveat stands); claims about LLM behavior (no LLM is invoked anywhere); any reopening of the 3-way sweep or Stage 3. R1 is bounded: invariance at widths {1,3,5} under content-blind serial replay — never "the axis is clean" in general.

**Budget and provenance (design §7).** $0 LLM/GPU/provider (sentinel-enforced); CPU wall ≤ 4 h under one monotonic deadline; measurement launched from the worktree root; all artifacts (profiles, envelopes, run DBs, replay outputs, comparison report) retained in an invocation-unique durable run directory and SHA-256-hashed in one manifest (nothing deleted), kept intact until independent acceptance review.

**External acceptance checks (owner-ratified 2026-08-17, binding — the rig does not self-certify these).** A measurement report is acceptable only if ALL three hold: (1) the report's spec echo equals the frozen `parrot_v2_spec` constants and exactly 45 run entries are present; (2) **every** violator triple shows `paired_served_differences ≥ 1` — *if this check fails, an internally reported R1 is not acceptable*; (3) every extracted cohort-window profile contains exactly 20 rounds.

**Amendment rule (local, append-only).** After this registration, any change to a frozen element above requires a dated amendment in this section plus a DECISIONS.md record **before** any measurement consumes it; once the first measurement run has consumed them, the frozen elements are immutable for that measurement and deviations are findings, never fixes. The measurement itself, any results closeout (append-only), and repository integration each require their own explicit, separately recorded authorization; none is granted by this registration.

**Measurement record (2026-08-17; append-only; outcome registered).** The authorized 45-run measurement executed from accepted commit `52cbbd9` terminated on run 1/45 with reading **R3** (auditable/reportable rig finding); the owner independently confirmed the failure chain, the report/manifest hashes, all 16 retained artifact hashes, the 20-round profiles, and the clean repository, then accepted the R3 outcome and closed the measurement. **No R1 or R2 is creditable.** Verified outcome: the first replay (schedule 818201, width 1) fail-stopped at round 1, schedule action 20 — a repeat refresh by agent 8 — on the platform return `{'success': False, 'error': 'UNIQUE constraint failed: trace.user_id, trace.created_at, trace.action, trace.info'}`. **Calibrated causal finding — a registered schedule–substrate incompatibility:** the registered uniform refresh-agent allocation produced a schedule that demonstrably contains same-agent same-round repeat refreshes (818201 round 1: agent 8 ×4, agent 36 ×4, twelve further agents ×2–3; with 50 draws over 50 agents repeated agents are highly probable, and this schedule's repeats are recorded fact, not a probability claim); OASIS's clock is round-granular and its trace table enforces `UNIQUE(user_id, created_at, action, info)`; the retained run database holds exactly one agent-8 refresh trace row at `created_at = 1`, and the rejected second refresh produced the identical primary-key tuple. The fail-closed replayer correctly refused to continue. The registered cohort never exercised this path (its trace rows are unique by construction and its LLM driver did not check refresh success); the rig's toy tests (1–2 refreshes/round) did not reach the repeat regime. **External acceptance checks:** (1) FAIL — no 45 run entries exist (the report's spec echo itself matches the frozen constants); (2) NOT EVALUABLE — no violator triple ran; (3) PASS — every extracted cohort-window profile contains exactly 20 rounds. **Rig contract held:** dispatch stopped at run 1; the partial manifest (16 artifacts) and the R3 report carrying the verbatim failure were persisted before the raise; nothing deleted; $0 spent; ~3 minutes of the 4 h ceiling. **Artifacts (durable, hashed):** `~/crowd-crit-runs/parrot_v2_invariance/` — `report_2026-08-17.json` SHA-256 `4c85cda1047914c48366ef38066420bc6a73e1397e3a047ccfb849bfb1592f8c`; `launch_2026-08-17.log` SHA-256 `77270a18b227886f3312e1e736244008dd33bc30910bddb923863ffa547916c7`; run dir `run-b04af8c4a4c04810b9c706fb7bb90efd/` with `manifest.json` SHA-256 `ab7a19509a394d7d2ed154552ec0177cf767ad9b47b6b86f8a4549d940f4ff54` over 16 artifacts; source-window DB SHA-256s recorded at preflight (`fc557f103c7e232a5389ff5ca41c111d9d79a9a4aa7f573e202f8e2b174cc284`, `e47fbf4dcaf1a369e68173449a39759774d8ef271b7a179f7be7722a650b98cb`, `8706c5601a79a30f86ac4e743f9b5c5e66dbe0239a1b5f47687a105a1afa00c1`). **The frozen elements above are unchanged by this record:** they were immutable for the measurement and remain unamended; any change to the allocation rule (or any other frozen element) for a future measurement requires a dated amendment here plus a DECISIONS.md record before that measurement, plus its own explicit authorization.

**Correction (2026-08-18; append-only; owner review of the measurement record).** The measurement record above states that the registered cohort "never exercised this path (its trace rows are unique by construction and its LLM driver did not check refresh success)." Neither formulation is supported by the evidence, and both are superseded by the following: **the banked cohort cannot establish whether this collision path occurred — retained traces contain only successful unique inserts (a failed duplicate insert leaves no trace row), while its LLM refresh consumer collapsed unsuccessful refresh returns into the no-posts state rather than failing closed** (`SocialEnvironment.get_posts_env` inspects `posts["success"]` but maps every unsuccessful return, errors included, to "no existing posts"; installed OASIS `social_agent/agent_environment.py:58–66`). The calibrated causal finding, the R3 outcome, the external-check results, and all artifact hashes are unchanged; only the cohort-comparison wording is corrected.
